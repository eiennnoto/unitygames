...
`<iframe src="https://...">` は Render 側の `/p/https/...` に書き換えられます。
その iframe がHTMLを読み込む場合、そのHTMLもこのプロキシを通るため、iframe内部の `script` / `img` / `link` / `iframe` / CSS / JavaScript内の絶対URLなども再度書き換えられます。

さらに `X-Frame-Options` と `Content-Security-Policy` のフレーム制約ヘッダーは上流からそのまま渡さないようにしているため、上流サイト側のフレーム制約でプロキシ表示が止まるケースを減らしています。
...
https://example.onrender.com/
  -> https://unityroom.com/
       -> iframe: https://78914.play.unityroom.com/
            -> iframe内のHTML
                 -> JS / 画像 / CSS / nested iframe
```

すべて同じ Render サーバーの `/p/https/...` 経由で取得されます（許可ホスト設定の範囲内）。
